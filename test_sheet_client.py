...
class FakeValues:
    ...
    def get(self, spreadsheetId, range):
        ...
        return _Executable({"values": self.grid})
...
class FakeService:
    def __init__(self, grid):
        self.values_api = FakeValues(grid)
...
def test_read_grid_returns_rows_from_the_named_tab():
    ...
    client = SheetClient(service, "SHEET_ID", "Sheet1")

    assert client.read_grid() == [["Title"], ["Alderbrook Hall"]]
    assert service.values_api.get_range == "Sheet1"
...
def test_write_cells_issues_a_single_batch_request():
    ...
    assert body["data"] == [
        {"range": "Sheet1!B2", "values": [["x"]]},
        {"range": "Sheet1!B3", "values": [["y"]]},
    ]
