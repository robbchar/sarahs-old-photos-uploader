...
mid-run-edit guard's fingerprint — see
...
1. **verify** — re-reads the Sheet and checks, per target, that its
   `file_template` columns still fingerprint the same photograph, that no
   OTHER row in the fresh read carries the same fingerprint (a duplicated
   fingerprint proves nothing about which physical row is underneath — see
   [`SHEET-PROTOCOL.md`](decisions/SHEET-PROTOCOL.md#a-fingerprint-only-proves-identity-while-it-is-unique)),
